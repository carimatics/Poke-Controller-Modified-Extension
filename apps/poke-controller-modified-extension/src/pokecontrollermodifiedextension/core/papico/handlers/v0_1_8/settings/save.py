...
class PapicoSettingsSaveHandler(PapicoHandler):
    ...
    def _save_settings(self) -> None:
        ...
        visible_software_controller = self._values["widget"]["software_controller"]["visible"]
        if visible_output1 and visible_output2 and visible_software_controller:
            config.set("Output", "widget_mode", "ALL (default)")
        else:
            vlist: list[str] = []
            if visible_output1: vlist.append("Output$1")
            if visible_output2: vlist.append("Output$2")
            if visible_software_controller: vlist.append("Software-Controller")
            widget_mode = " + ".join(vlist)
            if len(vlist) == 1:
                widget_mode = f"{widget_mode} Only"
            config.set("Output", "widget_mode", widget_mode)

        software_controller_position = self._values["widget"]["software_controller"]["position"]
        if software_controller_position == "top":
            software_controller_position = "1"
        else:
            software_controller_position = "2"
        config.set("Output", "software_controller_position", software_controller_position)

        dialogue_buttons_position = self._values["widget"]["dialog"]["confirm_buttons_position"]
        ...
        pokemon_home_single_or_double = self._values["external"]["pokemon_home"]["single_or_double"]
