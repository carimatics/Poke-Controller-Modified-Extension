...
KEY_MAP_JSON_ACTIONS: dict[str, dict[str, Any]] = {
    "button": {
        "y": SwitchButton.Y,
        "b": SwitchButton.B,
        "x": SwitchButton.X,
...
def parse_key_map_json(key_map_json: dict[str, dict[str, str]]) -> dict[Key | str, Any]:
    parsed: dict[Key | str, Any] = {}
    for kind, value in key_map_json.items():
        for action, key in value.items():
            if isinstance(key, str) and len(key) == 1:
                parsed[key] = KEY_MAP_JSON_ACTIONS[kind][action]
            elif isinstance(key, str):
                _, v = key.split(".")
                parsed[getattr(Key, v)] = KEY_MAP_JSON_ACTIONS[kind][action]
    return parsed


class SwitchKeyboard:
    def __init__(self, serial: Serial, key_map: dict[Key | str, Any]) -> None:
        self._serial = serial
    ...
    def _get_action(self, key: Key) -> Any | None:
        if key is None:
            return None
        if key.char in self._key_map:
            return self._key_map[key.char]
